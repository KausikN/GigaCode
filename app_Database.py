...
def SelectFileDialogBox_DBFile():
    ...
    if not os.path.splitext(root.filename)[-1] in ['.p', '.json']: # Check if selected file is pickle or json
        MainLabelText_DB.set("Please select a proper pickle/json File.")
        return
    else:
...
def SelectFileDialogBox_CreateDBFile():
    ...
    if os.path.splitext(DBFile_Path)[-1] == '.p':
        pickle.dump([], open(DBFile_Path, 'wb'))
    elif os.path.splitext(DBFile_Path)[-1] == '.json':
        json.dump({"Functions": []}, open(DBFile_Path, 'w'), sort_keys=True, indent=4)
...
def Add2DB():
    ...
    if not len(OpenedFiles) == 0:
        ...
        if os.path.splitext(DBFile_Path)[-1] == '.p':
            ...
            FunctionsData = json.load(open(DBFile_Path, 'r'))
            if 'Functions' in FunctionsData.keys():
        ...
        if os.path.splitext(DBFile_Path)[-1] == '.p':
            ...
            json.dump({"Functions": Functions}, open(DBFile_Path, 'w'), sort_keys=True, indent=4)
